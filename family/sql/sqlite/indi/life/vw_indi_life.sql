-- drop view family_vw_indi_life;

CREATE VIEW family_vw_indi_life as
select
	i.id, 
	i.sex, 
	p.givn,
	p.surn,
	coalesce(p.givn, '') || case when coalesce(p.givn, '') != '' and coalesce(p.surn, '') != '' then ' ' else '' end || coalesce(p.surn, '') as full_name,
