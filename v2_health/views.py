...
def chrono(request):
    ...
    return HttpResponseRedirect(reverse('v2_health:main'))
...
def item_form(request, pk):
    ...
    return HttpResponseRedirect(reverse('v2_health:main') + extract_get_params(request))
...
def main(request):
    ...
    if (request.method == 'POST'):
        if ('item-add' in request.POST):
            ...
            if item:
                return HttpResponseRedirect(reverse('v2_health:item_form', args = [item.id]))

    fixes = []
    fixes.append(Fix(CHRONO, _('chronology').capitalize(), 'v2/rok/icon/all.png', 'chrono/', None))
    fixes.append(Fix(BIOMARK, _('biomarkers').capitalize(), 'v2/rok/icon/all.png', 'biomark/', len(Biomarker.objects.filter(user = request.user.id))))
    fixes.append(Fix(INCIDENT, _('incidents').capitalize(), 'v2/rok/icon/all.png', 'incident/', len(Incident.objects.filter(user = request.user.id))))
    ...
    template = loader.get_template('v2_health/' + app_param.restriction + '.html')
