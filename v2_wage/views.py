...
from v2_hier.params import get_search_info, set_restriction, set_article_kind, set_article_visible, get_search_mode
...
from wage.models import app_name, set_active, Period, Depart, DepHist, Post, Employee, FioHist, Child, Appoint, Education, EmplPer, PayTitle, Payment
...
def main(request):
    ...
    fixes = []
    if (app_param.restriction in EMPL_ASIDE):
        fixes.append(Fix(PER, _('periods').capitalize(), 'v2/todo/icon/planned.png', 'periods/', len(Period.objects.filter(user = request.user.id))))
        fixes.append(Fix(EMPL_LIST, _('employees').capitalize(), 'v2/rok/icon/user.png', 'employees/', len(Employee.objects.filter(user = request.user.id))))
        fixes.append(Fix(EMPL_INFO, _('totals').capitalize(), 'v2/todo/icon/myday.png', 'total/', None))
        fixes.append(Fix(ACC, _('accruals').capitalize(), 'v2/todo/icon/myday.png', 'accrual/', len(Payment.objects.filter(employee = employee.id, period = period.id, direct = 0))))
        fixes.append(Fix(PAY, _('pays').capitalize(), 'v2/todo/icon/myday.png', 'payment/', len(Payment.objects.filter(employee = employee.id, period = period.id, direct = 1))))
        fixes.append(Fix(APP, _('appointments').capitalize(), 'v2/todo/icon/myday.png', 'appoint/', len(Appoint.objects.filter(employee = employee.id))))
        fixes.append(Fix(EDUC, _('educations').capitalize(), 'v2/todo/icon/myday.png', 'education/', len(Education.objects.filter(employee = employee.id))))
        fixes.append(Fix(SUR, _('surnames').capitalize(), 'v2/todo/icon/myday.png', 'surname/', len(FioHist.objects.filter(employee = employee.id))))
        fixes.append(Fix(CHLD, _('children').capitalize(), 'v2/todo/icon/myday.png', 'child/', len(Child.objects.filter(employee = employee.id))))
    elif (app_param.restriction in (DEP_HIST, DEP_INFO)):
        fixes.append(Fix(DEP_LIST, _('departments').capitalize(), 'v2/rok/icon/home.png', 'departments/', len(Depart.objects.filter(user = request.user.id))))
        fixes.append(Fix(DEP_HIST, _('department history').capitalize(), 'v2/rok/icon/home.png', 'dep_hist/', len(DepHist.objects.filter(depart = depart.id))))
    else:
        fixes.append(Fix(PER, _('periods').capitalize(), 'v2/todo/icon/planned.png', 'periods/', len(Period.objects.filter(user = request.user.id))))
        fixes.append(Fix(EMPL_LIST, _('employees').capitalize(), 'v2/rok/icon/user.png', 'employees/', len(Employee.objects.filter(user = request.user.id))))
        fixes.append(Fix(DEP_LIST, _('departments').capitalize(), 'v2/rok/icon/home.png', 'departments/', len(Depart.objects.filter(user = request.user.id))))
        fixes.append(Fix(POST, _('posts').capitalize(), 'v2/rok/icon/work.png', 'posts/', len(Post.objects.filter(user = request.user.id))))
        fixes.append(Fix(TITLE, _('pay titles').capitalize(), 'v2/rok/icon/edit.png', 'pay_titles/', len(PayTitle.objects.filter(user = request.user.id))))
        fixes.append(Fix(REPORT, _('reports').capitalize(), 'v2/rok/icon/news.png', 'reports/', 1))
    ...
    template = loader.get_template('v2_wage/' + app_param.restriction + '.html')
