...
def search(user, query):
    ...
    for item in items:
        result.add(app_name, 'cars', item.id, item.name, item.plate, False)

    lookups = Q(comment__icontains=query)
    items = Fuel.objects.filter(car__user = user.id).filter(lookups)
    for item in items:
        result.add(app_name, 'fuel', item.id, str(item), item.comment, True, item.car.name)

    lookups = Q(name__icontains=query) | Q(comment__icontains=query)
    items = Part.objects.filter(car__user = user.id).filter(lookups)
    for item in items:
        result.add(app_name, 'interval', item.id, item.name, item.info(), False, item.car.name)
    ...
    for item in items:
        result.add(app_name, 'service', item.id, str(item), item.info(), False, item.car.name)
