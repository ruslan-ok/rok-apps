...
from photo.models import app_name, Photo
...
def goto(request): # Опуститься в указанную папку
    ...
    return HttpResponseRedirect(reverse('v2_photo:main') + extract_get_params(request))
...
def do_main(request, restriction, pk = None, art_vis = False):
    ...
    if process_common_commands(request, app_name):
        return HttpResponseRedirect(reverse('v2_photo:' + restriction) + extract_get_params(request))
    ...
    if redirect_rest:
        return HttpResponseRedirect(reverse('v2_photo:' + redirect_rest) + extract_get_params(request))
