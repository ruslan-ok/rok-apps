...
def get_url_list(app):
    if (app == 'note'):
        return 'v2_note:note_list'
    else:
        return 'v2_news:news_list'

def get_url_form(app):
    if (app == 'note'):
        return 'v2_note:note_form'
    else:
        return 'v2_news:news_form'
...
def item_list(request, app):
    ...
    if (app_param.restriction == 'list') and (not app_param.lst):
        if (app == 'note'):
            return HttpResponseRedirect(reverse('v2_note:all_notes'))
        else:
            return HttpResponseRedirect(reverse('v2_news:all_news'))

    fixes = []
    fixes.append(Fix('all', _('all').capitalize(), 'v2/rok/icon/all.png', 'all/', len(Note.objects.filter(user = request.user.id, kind = app))))
    context['fix_list'] = fixes

    sorts = []
    sorts.append(Sort('name',  _('by name').capitalize(), 'v2/todo/icon/sort.png'))
    sorts.append(Sort('descr', _('by description').capitalize(), 'v2/rok/icon/note.png'))
    sorts.append(Sort('publ',  _('by publication date').capitalize(), 'v2/todo/icon/created.png'))
    ...
    if (app == 'note'):
        template_file = 'v2_note/note.html'
        context['add_item_placeholder'] = _('add note').capitalize()
    else:
        template_file = 'v2_note/news.html'
...
def get_file_storage_path(user, item, app):
    return storage_path_v2.format(user.id) + '{}/note_{}/'.format(app, item.id)
