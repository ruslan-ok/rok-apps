...
def search(user, query):
    ...
    for item in items:
        result.add(app_name, 'apart', item.id, item.name, item.addr, False)

    lookups = Q(info__icontains=query)
    items = Meter.objects.filter(apart__user = user.id).filter(lookups)
    for item in items:
        result.add(app_name, 'meter', item.id, item.name(), item.info, False, item.apart.name, item.period.strftime('%m.%Y'))

    lookups = Q(info__icontains=query) | Q(url__icontains=query)
    items = Bill.objects.filter(apart__user = user.id).filter(lookups)
    for item in items:
        result.add(app_name, 'bill', item.id, item.name(), item.info, False, item.apart.name, item.period.strftime('%m.%Y'))

    lookups = Q(name__icontains=query) | Q(abbr__icontains=query)
    items = Service.objects.filter(apart__user = user.id).filter(lookups)
    for item in items:
        result.add(app_name, 'service', item.id, item.name, item.abbr, False, item.apart.name)

    lookups = Q(info__icontains=query)
    items = Price.objects.filter(apart__user = user.id).filter(lookups)
    for item in items:
        result.add(app_name, 'price', item.id, item.name(), item.info, False, item.apart.name)
