...
from .models import app_name, Depart, Post, Employee, FioHist, Child, Appoint, Education, PayTitle, Payment
...
def search(user, query):
    ...
    for item in items:
        result.add(app_name, DEP_LIST, item.id, item.name, item.sort, False)

    lookups = Q(name__icontains=query)
    items = Post.objects.filter(user = user.id).filter(lookups)
    for item in items:
        result.add(app_name, POST, item.id, item.name, '', False)

    lookups = Q(name__icontains=query)
    items = PayTitle.objects.filter(user = user.id).filter(lookups)
    for item in items:
        result.add(app_name, TITLE, item.id, item.name, '', False)
    ...
    for item in items:
        ...
        result.add(app_name, EMPL_LIST, item.id, item.name(), item.info, False)

    lookups = Q(fio__icontains=query) | Q(info__icontains=query)
    items = FioHist.objects.filter(employee__user = user.id).filter(lookups)
    for item in items:
        result.add(app_name, SUR, item.id, str(item), item.info, False, item.employee.fio)
    ...
    for item in items:
        result.add(app_name, CHLD, item.id, str(item), item.info, False, item.employee.fio)
    ...
    for item in items:
        result.add(app_name, APP, item.id, item.name(), item.info, False, item.employee.fio)
    ...
    for item in items:
        result.add(app_name, EDUC, item.id, item.name(), item.info, False, item.employee.fio)
    ...
    for item in items:
        result.add(app_name, PAY, item.id, item.name(), item.info, False, item.employee.fio, item.period.name())
