...
#----------------------------------
TEMPLATES = {
    CARS: 'v2_fuel/car.html',
    FUEL: 'v2_fuel/fueling.html',
    INTR: 'v2_fuel/interval.html',
    SRVC: 'v2_fuel/service.html'
    }
#----------------------------------
...
def main(request):
    ...
    if (app_param.restriction != CARS) and (app_param.restriction != FUEL) and (app_param.restriction != INTR) and (app_param.restriction != SRVC):
        ...
        return HttpResponseRedirect(reverse('v2_fuel:main') + extract_get_params(request))
    ...
    if not Car.objects.filter(user = request.user.id, active = True).exists():
        if (app_param.restriction != CARS):
            ...
            return HttpResponseRedirect(reverse('v2_fuel:main'))
    ...
    if (request.method == 'POST'):
        ...
        if ('item-add' in request.POST):
            ...
            return HttpResponseRedirect(reverse('v2_fuel:item_form', args = [item_id]))
        if ('item-in-list-select' in request.POST) and (app_param.restriction == CARS):
            ...
            if pk:
                ...
                return HttpResponseRedirect(reverse('v2_fuel:item_form', args = [pk]))
    ...
    fixes = []
    fixes.append(Fix(CARS, _('cars').capitalize(), 'v2/rok/icon/car.png', 'cars/', len(Car.objects.filter(user = request.user.id))))
    fixes.append(Fix(FUEL, _('fuelings').capitalize(), 'v2/rok/icon/gas.png', 'fuels/', len(Fuel.objects.filter(car = car))))
    fixes.append(Fix(INTR, _('service intervals').capitalize(), 'v2/todo/icon/remind-today.png', 'intervals/', len(Part.objects.filter(car = car))))
    fixes.append(Fix(SRVC, _('repair and service').capitalize(), 'v2/todo/icon/myday.png', 'services/', len(Repl.objects.filter(car = car))))
