...
class SiteService():
    ...
    def log_event_api(self, type, name, info):
        ...
        api_url = f'{api_host}/api/logs/?format=json'
    ...
    def get_events(self, app=None, service=None, type=None, name=None, day=None, order_by=None, local=False):
        if not order_by:
            order_by = '-created'

        if self.use_log_api and not local:
