...
from trip.models import app_name, Trip, Saldo, Person, trip_summary, set_active, PERS, TRIP
...
def get_template_file(restriction):
    if (restriction == PERS):
        return 'v2_trip/person.html'
    return 'v2_trip/trip.html'
...
def main(request):
    ...
    if (app_param.restriction != PERS) and (app_param.restriction != TRIP):
        ...
        return HttpResponseRedirect(reverse('v2_trip:main') + extract_get_params(request))
    ...
    if (request.method == 'POST'):
        if ('item-add' in request.POST):
            ...
            return HttpResponseRedirect(reverse('v2_trip:item_form', args = [item_id]))
        ...
        if ('item-in-list-select' in request.POST) and (app_param.restriction == PERS):
            ...
            if pk:
                ...
                return HttpResponseRedirect(reverse('v2_trip:item_form', args = [pk]))
    ...
    fixes = []
    fixes.append(Fix(PERS, _('persons').capitalize(), 'v2/rok/icon/user.png', 'persons/', len(Person.objects.filter(user = request.user.id))))
    fixes.append(Fix(TRIP, _('trips').capitalize(), 'v2/rok/icon/car.png', 'trips/', len(Trip.objects.filter(user = request.user.id))))
