...
, cte_todo_termin as (
	SELECT
		...
		CASE WHEN r.num_role != 1 OR t.stop IS NULL THEN NULL ELSE (t.stop::date - CURRENT_DATE)::integer END as termin,
		t.completed,
		t.task_2_id,
		t.price_service,
		STRING_AGG(CONCAT(bi.is_base, '_', b.app, '_', b.num_role, '_', COALESCE(bi.icon, 'search-results')), ',') as related
	FROM cte_roles r
	JOIN task_task t
		ON r.id = t.id
	...
	GROUP BY
		...
		CASE WHEN r.num_role != 1 OR t.stop IS NULL THEN NULL ELSE (t.stop::date - CURRENT_DATE)::integer END,
		t.completed,
		t.task_2_id,
		t.price_service
)
, cte_subgroup_id as (
	SELECT
		...
		t.price_service,
		CASE 
			WHEN t.num_role = 1 AND t.completed THEN 6
...
, cte_task_info as (
	SELECT
		t.id as task_id,
        CONCAT(SUBSTR(REPLACE(REPLACE(t.info, CHR(13), ' '), CHR(10), ''), 1, 80), CASE WHEN LENGTH(REPLACE(REPLACE(t.info, CHR(13), ' '), CHR(10), '')) > 80 THEN '...' ELSE '' END) as task_descr,
