...
function sendTokenToServer(currentToken) {
  if (!isTokenSentToServer() || tokenChanged(currentToken)) {
    ...
    const url = 'https://rok-apps.com/todo/fcm_add?token=' + currentToken;
    fetch(url)
...
function delTokenOnServer() {
  if (isTokenSentToServer()) {
    ...
    if (oldToken) {
      ...
      const url = 'https://rok-apps.com/todo/fcm_del?token=' + oldToken;
      fetch(url)
        .then(function(response) { 
          return response.text().then(function(text) {
            console.log('Request succeeded with response "' + text + '"');
...
navigator.serviceWorker.register('https://rok-apps.com/firebase-messaging-sw.js');
