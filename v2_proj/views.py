...
from proj.models import app_name, Projects, set_active, Expenses, s_proj_summary, PROJ, EXPN
...
#----------------------------------
TEMPLATES = {
    PROJ: 'v2_proj/project.html',
    EXPN: 'v2_proj/expense.html'
    }
...
def main(request):
    ...
    if (app_param.restriction != PROJ) and (app_param.restriction != EXPN):
        ...
        return HttpResponseRedirect(reverse('v2_proj:main') + extract_get_params(request))
    ...
    if not Projects.objects.filter(user = request.user.id, active = True).exists():
        if (app_param.restriction != PROJ):
            ...
            return HttpResponseRedirect(reverse('v2_proj:main'))
    ...
    if (request.method == 'POST'):
        ...
        if ('item-add' in request.POST):
            ...
            return HttpResponseRedirect(reverse('v2_proj:item_form', args = [item_id]))
        if ('item-in-list-select' in request.POST) and (app_param.restriction == PROJ):
            ...
            if pk:
                ...
                return HttpResponseRedirect(reverse('v2_proj:item_form', args = [pk]))
    ...
    fixes = []
    fixes.append(Fix(PROJ, _('projects').capitalize(), 'v2/todo/icon/myday.png', 'projects/', len(Projects.objects.filter(user = request.user.id))))
    fixes.append(Fix(EXPN, _('expenses').capitalize(), 'v2/rok/icon/cost.png', 'expenses/', len(Expenses.objects.filter(direct = project))))
    ...
    if (app_param.restriction == EXPN):
        sorts.append(Sort('date', _('by date').capitalize(), 'v2/rok/icon/application.png'))
        sorts.append(Sort('kontr', _('by contractor').capitalize(), 'v2/rok/icon/car.png'))
        sorts.append(Sort('text', _('by description').capitalize(), 'v2/rok/icon/note.png'))
    sorts.append(Sort('created', _('by creation date').capitalize(), 'v2/todo/icon/created.png'))
    sorts.append(Sort('lmod',  _('by last modification date').capitalize(), 'v2/todo/icon/planned.png'))
