...
def todo_base_context(request):
    ...
    context['list_url'] = 'v2_todo:' + app_param.restriction
    if (app_param.restriction == MY_DAY):
        context['title_info'] = datetime.today().strftime('%a, %d %B')

    fixes = []
    fixes.append(Fix('myday', _('my day').capitalize(), 'v2/todo/icon/myday.png', 'myday/', len(get_tasks(request.user, MY_DAY, 0).exclude(completed = True))))
    fixes.append(Fix('important', _('important').capitalize(), 'v2/todo/icon/important.png', 'important/', len(get_tasks(request.user, IMPORTANT, 0))))
    fixes.append(Fix('planned', _('planned').capitalize(), 'v2/todo/icon/planned.png', 'planned/', len(get_tasks(request.user, PLANNED, 0))))
    fixes.append(Fix('all', _('all').capitalize(), 'v2/rok/icon/all.png', 'all/', len(get_tasks(request.user, ALL, 0))))
    fixes.append(Fix('completed', _('completed').capitalize(), 'v2/todo/icon/completed.png', 'completed/', len(get_tasks(request.user, COMPLETED, 0))))
    context['fix_list'] = fixes

    sorts = []
    if (app_param.restriction != MY_DAY):
        sorts.append(Sort('myday', _('my day').capitalize(), 'v2/todo/icon/myday.png'))
    if (app_param.restriction != IMPORTANT):
        sorts.append(Sort('important', _('by importance').capitalize(), 'v2/todo/icon/important.png'))
    sorts.append(Sort('termin', _('by termin date').capitalize(), 'v2/todo/icon/planned.png'))
    if (app_param.restriction in (COMPLETED, LIST_MODE)):
        sorts.append(Sort('completion', _('by date of completion').capitalize(), 'v2/todo/icon/completed.png'))
    sorts.append(Sort('name',  _('by name').capitalize(), 'v2/todo/icon/sort.png'))
...
def task_list(request):
    ...
    if process_common_commands(request, app_name):
        return HttpResponseRedirect(reverse('v2_todo:task_list') + extract_get_params(request))
    ...
    if (request.method == 'POST'):
        ...
        if 'item-add' in request.POST:
            ...
            return HttpResponseRedirect(reverse('v2_todo:task_form', args = [task.id]))
        if 'list-add' in request.POST:
            lst_id = list_add(request.user, app_name, request.POST['name'])
            return HttpResponseRedirect(reverse('v2_todo:list_form', args = [lst_id]))
        if 'group-add' in request.POST:
            grp_id = group_add(request.user, app_name, request.POST['name'])
            return HttpResponseRedirect(reverse('v2_todo:group_form', args = [grp_id]))
    ...
    if app_param.article:
        if (app_param.kind == 'task'):
            if not Task.objects.filter(id = app_param.art_id, user = request.user.id).exists():
                ...
                template_file = 'v2_todo/task.html'
...
def all_tasks(request):
    ...
    return HttpResponseRedirect(reverse('v2_todo:task_list'))
